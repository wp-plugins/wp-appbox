<?php
	...
	class GetAppInfoAPI {
		...
		function getGooglePlay($item_id) {
			...
			if((isset($this_content[0])) && ($this_content[1]['http_code'] == '200')) {
				...
				foreach(pq('img[itemprop="screenshot"]') as $appshots) {
					...
					if(substr($app_screen_shots, -5, 5) == '=h230') $app_screen_shots = str_replace('=h230', '', $app_screen_shots);
		...
		function getFirefoxMarketplace($item_id) {
			if($this->is_cached($item_id)) return($this->get_cached_data($item_id));
			$page_url = 'https://marketplace.firefox.com/api/v1/apps/app/'.$item_id.'/?region=de&lang=de';
			$this_content = $this->get_fcontent(urldecode($page_url));
		...
		function getAmazonApps($item_id) {
			if($this->is_cached($item_id)) return($this->get_cached_data($item_id));
			$thestoreurl = 'http://www.amazon.de/gp/product/';
			$page_url = $thestoreurl.$item_id.'/?ie=UTF8';
			$this_content = $this->get_fcontent(urldecode($page_url));
			if(WPAPPBOX_ERRROR_PARSEOUTPUT == '1') print_r($this_content);
			if((isset($this_content[0])) && ($this_content[1]['http_code'] == '200')) {
				phpQuery::newDocumentHTML($this_content[0]);
		...
		function getAppStore($item_id) {
			...
			if($this_content->resultCount == '1') {
				...
				if(!$options['wpAppbox_itunes_secureimage']) {
					if(preg_match('~http://.*\.(([0-9]{2,4})x[0-9]{2,4}-[0-9]{2,3}).png~i', $banner_icon, $replacement)) {
						$banner_icon = str_replace($replacement[1], '128x128-75', $banner_icon);
					}
					else $banner_icon = str_replace('.png', '.128x128-75.png', $banner_icon);
				}
		...
		function getWordPress($item_id) {
			if($this->is_cached($item_id)) return($this->get_cached_data($item_id));
			$thestoreurl = 'http://wordpress.org/plugins/';
			$page_url = $thestoreurl.$item_id.'/';
		...
		function getOperaAddons($item_id) {
			//if($this->is_cached($item_id)) return($this->get_cached_data($item_id));
			$page_url = 'https://addons.opera.com/de/extensions/details/'.$item_id.'/?display=de';
			$this_content = $this->get_fcontent(urldecode($page_url));
			if(WPAPPBOX_ERRROR_PARSEOUTPUT == '1') print_r($this_content);
			if((isset($this_content[0])) && ($this_content[1]['http_code'] == '200')) {
				phpQuery::newDocumentHTML($this_content[0]);
