		...
		function theAppLink_AppStore($link) {
			...
			if($options['wpAppbox_user_affiliateids']) {
				...
				if($options['wpAppbox_user'.$author_id.'_tradedoubler_global']) {
					$affid = ($options['wpAppbox_user'.$author_id.'_tradedoubler_sponsored'] == true) ? $dev_id : $options['wpAppbox_user'.$author_id.'_tradedoubler'];
				}
			}
		...
		function theOutput($attr) {
			if($this->checkAttributs($attr) === true) {
				...
				if($app_info === 0) return($this->errorOutput('notfound', $attr['store'], $attr['appid']));
					...
					if(($attr['style'] == 'screenshots') || ($attr['style'] == 'screenshots-only')) $template = str_replace('{SCREENSHOTS}', $this->theScreenshots($app_screenshots), $template);
