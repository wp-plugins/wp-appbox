<?php
	...
	class CreateOutput {
		...
		function theReloadLink($app_data) {
			if(get_option('wpAppbox_showReload') && wpAppbox_isUserAuthor()) return('<a href="'.get_permalink().(is_preview() ? '&amp;' : '?').'wpappbox_reload_cache&amp;wpappbox_appid='.$app_data->app_id.'" title="'.__('Renew cached data of this app', 'wp-appbox').'" class="reload-link">&#8635;</a> ');
		}
		
		function errorOutput($error_type, $storeID = '', $app_id = '', $style = 'simple') {
			switch($error_type) {
				case 'nostore':
			...
			if($error_type != 'fallback') $output = '<!-- WP-Appbox (Store: '.$storeID.' // ID: '.$app_id.') --><div class="wpappbox error"><span>'.$output.' :-( #wpappbox</span></div><!-- /WP-Appbox -->';
			if((get_option('wpAppbox_eOnlyAuthors') == false) || wpAppbox_isUserAuthor() || isset($_GET['wpappbox_show_errormessages'])) return($output);
		}
