<?php
	...
	class CreateOutput {
		...
		function theAppIcon($app_data, $app_store) {
			...
			if(($app_data->icon_bg != '') && ($app_data->storename_css == 'windowsstore')) return($app_icon.'" style="'.$app_data->icon_bg);
		...
		function theScreenshots($app_screenshots, $app_id, $app_store) {
			...
			foreach($app_screenshots as $appshots) {
				...
				$screenshot_url = str_replace('http://', '//', $screenshot_url);
				$screenshots_out .= '<li><img src="'.$this->cleanURL($screenshot_url, $app_store).'" alt="" /></li>';
			}
		...
		function theReloadLink($app_data) {
			$options = get_option('wpAppbox');
			if($options['wpAppbox_show_reload_link'] && has_user_permissions()) return('<a href="'.get_permalink().'?wpappbox_reload_cache&amp;wpappbox_appid='.$app_data->app_id.'" title="'.__('Renew cached data of this app', 'wp-appbox').'" class="reload-link">&#8635;</a> ');
		}
				...
				case 'fallback':
					...
					switch($store) {
						...
						case 'steam':
							...
							else $fallback_descr = __('The game requires an age verification on the Steam website. Therefore, unfortunately, no further information can be displayed.', 'wp-appbox');
							break;
					}
		...
		function theOutput($attr) {
			...
			if($this->checkAttributs($attr) === true) {
				...
				if($app_info === 0) return($this->errorOutput('notfound', $attr['store'], $attr['appid']));
					...
					if(($attr['style'] == 'screenshots') || ($attr['style'] == 'screenshots-only')) $template = str_replace('{SCREENSHOTS}', $this->theScreenshots($app_screenshots, $this->theAppID($app_data), $app_store), $template);
