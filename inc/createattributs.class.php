<?php
	...
	class CreateAttributs {
		...
		function getStandardStyle($store) {
			global $style_names_global;
			$options = get_option('wpAppbox');
			if($options['wpAppbox_view_'.$store] == '-1') return($style_names_global[$options['wpAppbox_view_default']]);
		...
		function devideAttributs($attribute) {
			...
			if(is_array($attribute)) {
				foreach ($attribute as $value) {
					if($this->isValueNoRCode($value)) $attr['showqrcode'] = false;
