		...
		function get_store_url($store, $id) {
			switch($store) {
				...
				case 'chromewebstore':
					...
					else $url = 'https://chrome.google.com/webstore/detail/{APPID}';
					break;		
				case 'intelappup':
		...
		function getAppStore($item_id) {
			...
			if($this_content->resultCount == '1') {
				...
				if(preg_match('~http://.*\.(tif)~i', $this_content->artworkUrl100)) $banner_icon = $this_content->artworkUrl60;
					...
					if(trim($banner_icon) == '') $banner_icon = $this_content->artworkUrl60;
					if(($fool == 'fool') && (!$options['wpAppbox_itunes_secureimage']) && ($this_content->kind != 'podcast')) {
