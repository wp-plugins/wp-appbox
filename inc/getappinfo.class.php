<?php
	...
	class GetAppInfoAPI {
		...
		function echo_parse_output() {
			if(get_option('wpAppbox_eOutput') && has_user_admin_permissions()) return true;
			else false;
		}
		...
		function get_store_url($storeID, $appID) {
			global $wpAppbox_storeURL;
			if((get_option('wpAppbox_storeURL_'.$storeID) == '1') || (get_option('wpAppbox_storeURL_'.$storeID) == '')) $url = $wpAppbox_storeURL[$storeID][1];
			elseif((get_option('wpAppbox_storeURL_'.$storeID) == '0') && (get_option('wpAppbox_storeURL_URL_'.$storeID) != '')) $url = get_option('wpAppbox_storeURL_URL_'.$storeID);
			elseif((get_option('wpAppbox_storeURL_'.$storeID) == '0') && (get_option('wpAppbox_storeURL_URL_'.$storeID) == '')) $url = $wpAppbox_storeURL[$storeID][1];
			else $url = $wpAppbox_storeURL[$storeID][get_option('wpAppbox_storeURL_'.$storeID)];
			$url = str_replace('{APPID}', $appID, $url);
			$url = str_replace('{ID}', $appID, $url);
			return($url);
		}
		
		
		function get_cache_name($id) {
			return(WPAPPBOX_PREFIX.md5($id));
		}
		...
		function save_data_to_cache($id, $appinfo) {
			...
			foreach($appinfo['General'] as $appdata) {
				if(trim($appdata->app_title) != '') set_transient($this->get_cache_name($id), $appinfo, WPAPPBOX_CACHINGTIME*60);
			}
		}
		
		
		function get_fcontent($url, $javascript_loop = 0, $timeout = 3) {
			if(get_option('wpAppbox_curlTimeout') != '') $timeout = get_option('wpAppbox_curlTimeout');
		...
		function getAmazonApps($item_id) {
			...
			if((isset($this_content[0])) && ($this_content[1]['http_code'] == '200')) {
				...
				$app_price = pq('#actualPriceValue>strong')->html();
		...
		function getAppStore_old($item_id) {
			...
			if($this_content->resultCount == '1') {
				...
				if(preg_match('~http://.*\.(tif)~i', $this_content->artworkUrl100)) $app_icon = $this_content->artworkUrl60;
					...
					if((get_option('wpAppbox_eImageApple') == false) && ($this_content->kind != 'podcast')) {
