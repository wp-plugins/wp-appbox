	...
	class GetAppInfoAPI {
		...
		function get_store_url($store, $id) {
			switch($store) {
				...
				case 'intelappup':
					...
					else $url = 'http://www.appup.com/de/app-details/{APPID}';
					break;					
			}
		...
		function getAmazonApps($item_id) {
			...
			if((isset($this_content[0])) && ($this_content[1]['http_code'] == '200')) {
				...
				$app_rating = substr(pq('span.rating-stars span.swSprite')->attr('title'), 0, 3);
				$app_title = pq('span#btAsinTitle>span')->html();
		...
		function getIntelAppUp($item_id) {
			if($this->is_cached($item_id)) return($this->get_cached_data($item_id));
			$page_url = $this->get_store_url('intelappup', $item_id);
			$this_content = $this->get_fcontent(urldecode($page_url));
			if($this->echo_parse_output()) print_r($this_content);
			if((isset($this_content[0])) && ($this_content[1]['http_code'] == '200')) {
				phpQuery::newDocumentHTML($this_content[0]);
				$error_found = pq('a.details-buy-btn')->attr('wa_name');
				if($error_found == '') return 0;
				$app_rating = pq('div[itemprop="aggregateRating"]>img[itemprop="ratingValue"]')->attr('content');
				$banner_icon = pq('meta[property="og:image"]')->attr('content');
				$app_title = pq('meta[property="og:title"]')->attr('content');
				$app_store_url = pq('meta[property="og:url"]')->attr('content');
				$app_author = pq('span[itemprop="publisher"]>meta[itemprop="name"]')->attr('content');
				$author_store_url = 'http://www.appup.com'.pq('span[itemprop="publisher"]>meta[itemprop="url"]')->attr('content');
				if(pq('div#app-info>div#app-price>span[itemprop="offers"]>meta[itemprop="price"]')->attr('content') == '0.00') $app_price = __('Kostenlos', 'wp-appbox');
				else $app_price = trim(pq('div#app-info>div#app-price"]')->html());
				foreach(pq('div#mod-screenshot-carousel>ul.thumbnail>li>img') as $appshots) {
					$app_screen_shots = 'http://www.appup.com'.pq($appshots)->attr('src');
					$app_info['ScreenShots'][] = (object) array('screen_shot' => $app_screen_shots);
				}
				$app_store_url = $page_url;
				$app_info['General'][] = (object) array(
					...
					'app_price' => $app_price,
					'app_rating' => $app_rating,
					'storename' => 'Intel AppUp',
					'storename_css' => 'intelappup',
					'fallback' => '0'
				);
				$this->save_data_to_cache($item_id, $app_info);
				return $app_info;
