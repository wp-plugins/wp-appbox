...
function wpAppbox_options_page() {
	...
	<div class="wrap">
		...
		<div class="widget" style="margin:15px 0;"><p style="margin:10px;">
			<a href="http://www.blogtogo.de/wp-appbox-app-badge-fuer-google-play-mac-app-store-windows-store-windows-phone-store-co/" target="_blank"><?php _e('Besuch die Plugin-Seite', 'wp-appbox'); ?></a> | <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=SH9AAS276RAS6" target="_blank"><?php _e('Spenden via PayPal', 'wp-appbox'); ?></a> | <a href="http://wordpress.org/extend/plugins/wp-appbox/" target="_blank"><?php _e('Plugin im WordPress Directory', 'wp-appbox'); ?></a> | <a href="https://twitter.com/Marcelismus" target="_blank"><?php _e('Folge mir via Twitter', 'wp-appbox'); ?></a>
	...
	switch($tab){
		case 'general':
			...
			<tr valign="top">
				...
				<td>
					<input type="checkbox" name="wpAppbox_nofollow" value="1" <?php  if($options['wpAppbox_nofollow'] == true) echo 'checked="checked"'; ?> /> <span class="description"><?php _e('Fügt allen Links das Attribut "<a href="http://de.wikipedia.org/wiki/Nofollow" target="_blank">nofollow</a>" hinzu', 'wp-appbox'); ?></span>
				</td>
			</tr>
			<th scope="row"><?php _e('Öffne Links in einem neuen Fenster', 'wp-appbox'); ?>:</th>
				<td>
					<input type="checkbox" name="wpAppbox_blank" value="1" <?php  if($options['wpAppbox_blank'] == true) echo 'checked="checked"'; ?> /> <span class="description"><?php _e('Öffnet alle App-Links in einem neuen Fenster (target="blank")', 'wp-appbox'); ?></span>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row" colspan="2"><strong><?php _e('Fehlerausgabe', 'wp-appbox'); ?></strong><br /><i><?php _e('Die Fehlerausgabe sollte nur in Problemfällen eingeschaltet werden. Kann das Design auseinandernehmen.', 'wp-appbox'); ?></i></th>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Parse Output', 'wp-appbox'); ?>:</th>
				<td>
					<input type="checkbox" name="wpAppbox_error_parseoutput" value="1" <?php  if($options['wpAppbox_error_parseoutput'] == true) echo 'checked="checked"'; ?> /> <span class="description"><?php _e('Parse Output aktivieren', 'wp-appbox'); ?></span>
				</td>
			</tr>
			<?php
		break;
		case 'editor':
			...
			<tr valign="top">
				<th scope="row"><?php _e('AndroidPit', 'wp-appbox'); ?>:</th>
				<td>
			...
			<tr valign="top">
				<th scope="row"><?php _e('Chrome Web Store', 'wp-appbox'); ?>:</th>
				<td>
