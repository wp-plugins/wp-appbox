	...
	class Cached_Apps extends WP_List_Table {
		...
		function fill_data($search) {
			...
			foreach($apps_infos as $key => $app) {
				...
				$apps[$option_name] = array('app_title' => trim($app['General'][0]->app_title),
		...
		function column_default($item, $column_name) {
			...
			switch($column_name) { 
				...
				case 'app_icon':
					if(($item['app_store'] == 'windowsstore') && ($item['app_icon_bg'] != '')) return '<img src="'.$item[$column_name].'" style="background-color: '.$item['app_icon_bg'].'; width:48px; height:48px;" />';
				...
				case 'app_store':
					...
					return '<img src="'.plugins_url('img/'.(($item[$column_name] == 'macappstore') ? 'macappstore' : $item[$column_name]).'-small.png', dirname(__FILE__)).'" style="margin-right:8px; height:14px;" />'.(($item[$column_name] == 'macappstore') ? '(Mac) App Store' : $wpAppbox_storeNames[$item[$column_name]]);
				case 'app_timeout':
		...
		function prepare_items() {
			...
			if($this->is_active_search()) {
				$apps = $this->fill_data($_POST['s']);
			}
			else $apps = $this->fill_data(); 
		...
		function process_bulk_action() {
			...
			if(!empty($app_transient_ids)) {
				...
				if('reload' === $this->current_action()) {
					foreach($app_transient_ids as $id) {
						$app_data = get_transient(WPAPPBOX_PREFIX.$id);
		...
		function no_items() {
			if($this->is_active_search()) $theecho = __('No apps in WordPress-cache were found with this term.', 'wp-appbox').' '.'<a href="/wp-admin/options-general.php?page=wp-appbox&tab=cache">'.__('Clear search', 'wp-appbox').'</a>';
