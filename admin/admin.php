...
function wpAppbox_adminInit() {
	...
	if(!array_key_exists('wpAppbox_affiliate_apple_service', $settings)) $settings['wpAppbox_affiliate_apple_service'] = 'tradedoubler';
	...
	if(!array_key_exists('wpAppbox_view_kickstarter', $settings)) $settings['wpAppbox_view_kickstarter'] = '0';
	...
	if(!array_key_exists('wpAppbox_view_intelappup', $settings)) $settings['wpAppbox_view_intelappup'] = '0';
	...
	if($settings['wpAppbox_view_androidpit'] == '-1') $settings['wpAppbox_view_androidpit'] = '0';
	...
	if($settings['wpAppbox_view_intelappup'] == '-1') $settings['wpAppbox_view_intelappup'] = '0';
...
function wpAppbox_options_page() {
	...
	<div class="wrap">
		...
		<?php 
			if($options['wpAppbox_notification'] != '234') {
				echo '<div id="setting-error-settings_updated" class="updated settings-error"><p><strong>'.__('<a href="wp-appbox-2-3-0-bringt-unterstuetzung-fuer-good-old-games-gog-com-mit" target="_blank">WP-Appbox 2.3.0</a> ist da! Falls Dir das Plugin gefällt, <a href="http://wordpress.org/support/view/plugin-reviews/wp-appbox" target="_blank">bewerte es doch im Plugin-Verzeichnis</a>. :-)', 'wp-appbox').'</strong></p></div>'; 
				$options['wpAppbox_notification'] = '234';
