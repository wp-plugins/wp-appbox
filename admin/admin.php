...
function wpAppbox_adminInit() {
	...
	if(!array_key_exists('wpAppbox_view_default', $settings)) $settings['wpAppbox_view_default'] = '0';
	if(!array_key_exists('wpAppbox_view_appstore', $settings)) $settings['wpAppbox_view_appstore'] = '-1';
	if(!array_key_exists('wpAppbox_view_googleplay', $settings)) $settings['wpAppbox_view_googleplay'] = '-1';
	if(!array_key_exists('wpAppbox_view_androidpit', $settings)) $settings['wpAppbox_view_androidpit'] = '-1';
	if(!array_key_exists('wpAppbox_view_windowsstore', $settings)) $settings['wpAppbox_view_windowsstore'] = '-1';
	if(!array_key_exists('wpAppbox_view_windowsphone', $settings)) $settings['wpAppbox_view_windowsphone'] = '-1';
	if(!array_key_exists('wpAppbox_view_chromewebstore', $settings)) $settings['wpAppbox_view_chromewebstore'] = '-1';
	if(!array_key_exists('wpAppbox_view_amazonapps', $settings)) $settings['wpAppbox_view_amazonapps'] = '-1';
	if(!array_key_exists('wpAppbox_view_intelappup', $settings)) $settings['wpAppbox_view_intelappup'] = '-1';
	if(!array_key_exists('wpAppbox_view_firefoxaddon', $settings)) $settings['wpAppbox_view_firefoxaddon'] = '-1';
	if(!array_key_exists('wpAppbox_view_firefoxmarketplace', $settings)) $settings['wpAppbox_view_firefoxmarketplace'] = '-1';
	if(!array_key_exists('wpAppbox_view_operaaddons', $settings)) $settings['wpAppbox_view_operaaddons'] = '-1';
	if(!array_key_exists('wpAppbox_view_samsungapps', $settings)) $settings['wpAppbox_view_samsungapps'] = '-1';
	if(!array_key_exists('wpAppbox_view_steam', $settings)) $settings['wpAppbox_view_steam'] = '-1';
	if(!array_key_exists('wpAppbox_view_wordpress', $settings)) $settings['wpAppbox_view_wordpress'] = '-1';
...
function wpAppbox_options_page() {
	...
	<div class="wrap">
		...
		<?php 
			if($options['wpAppbox_notification'] != '2') {
				echo '<div id="setting-error-settings_updated" class="updated settings-error"><p><strong>'.__('<a href="http://www.blogtogo.de/wp-appbox-2-0-0-is-here-danke-fuer-10-000-downloads/" target="_blank">WP-Appbox 2.0.0</a> ist da! Danke für 10.000 Downloads! Falls Dir das Plugin gefällt, <a href="http://wordpress.org/support/view/plugin-reviews/wp-appbox" target="_blank">bewerte es doch im Plugin-Verzeichnis</a>. :-)', 'wp-appbox').'</strong></p></div>'; 
				$options['wpAppbox_notification'] = '2';
		...
		<?php
			...
			<div style="position: relative;width:100%; height: auto; text-align: center; background-color:#f5d6a9;">
				<div style="position: absolute; bottom: 4px; right: 8px;"><small>Logo by <a href="https://twitter.com/craive" target="_blank">@craive</a></small></div>
