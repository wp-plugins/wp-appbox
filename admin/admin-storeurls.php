		...
		<tr valign="top">
			...
			<td>	
				...
				?>
					...
					<input <?php if(get_option('wpAppbox_storeURL_'.$storeID) != 0) echo 'disabled="disabled"'; ?> type="text" value="<?php echo(get_option('wpAppbox_storeURL_'.$storeID) == '0') ? get_option('wpAppbox_storeURL_'.$storeID.'_URL') : $wpAppbox_storeURL[$storeID][get_option('wpAppbox_storeURL_'.$storeID)]; ?>" name="wpAppbox_storeURL_<?php echo($storeID); ?>_URL" id="wpAppbox_storeURL_<?php echo $store; ?>_URL" style="width:500px;" />
				<?php } ?>
			</td>
		</tr>
	<?php } ?>
