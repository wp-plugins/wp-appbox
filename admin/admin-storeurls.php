			...
			<td>	
				...
				?>
					<select style="width: 185px;" onChange="javascript:show_hide_box('<?php echo $store; ?>')" name="wpAppbox_storeurl_<?php echo $store; ?>" id="wpAppbox_storeurl_<?php echo $store; ?>" class="postform">
						<option <?php selected($options['wpAppbox_storeurl_'.$store], '0'); ?> value="0" data="<?php echo $options['wpAppbox_storeurl_'.$store.'_url']; ?>"><?php echo $store_urls_languages[0]; ?></option>
						<?php 
							asort($store_urls_languages);
							foreach($store_urls_languages as $id => $language) { ?>
							<?php if(($id != '0') && ($store_urls[$store][$id] != '')) { ?>
								<option <?php selected($options['wpAppbox_storeurl_'.$store], $id); ?> value="<?php echo $id; ?>" data="<?php echo $store_urls[$store][$id]; ?>"><?php echo $language; ?></option>
							<?php } ?>
						<?php } ?>
					</select>
