<?php function wpAppbox_add_custom_affiliate_id($user) { ?>
		...
		<table class="form-table">
			<tr>
				<th>
					<label for="wpAppbox_user<?php echo($userid); ?>_tradedoubler"><?php _e('TradeDoubler-ID', 'wpappbox'); ?>:</label>
				</th>
				<td>
					...
					<div id="wpAppbox_user<?php echo($userid); ?>_tradedoubler_div" <?php if(!$options['wpAppbox_user'.$userid.'_tradedoubler_global']) echo 'style="display:none;"'; ?>>
						<p style="margin-left: 24px;"><input type="text" name="wpAppbox_user<?php echo($userid); ?>_tradedoubler" id="wpAppbox_user<?php echo($userid); ?>_tradedoubler" value="<?php echo $options['wpAppbox_user'.$userid.'_tradedoubler']; ?>" class="regular-text" style="width: 200px;" /> <span class="description"><?php _e('Deine Affiliate-ID bei TradeDoubler. Wird keine ID angegeben, wird die Standard-ID genutzt.', 'wpappbox'); ?></span></p>
						<p style="margin-left: 24px;"><label for="wpAppbox_user<?php echo($userid); ?>_tradedoubler_sponsored"><input type="checkbox" name="wpAppbox_user<?php echo($userid); ?>_tradedoubler_sponsored" value="1" <?php if($options['wpAppbox_user'.$userid.'_tradedoubler_sponsored']) echo 'checked="checked"'; ?> /> <?php _e('Ich habe keine ID bei TradeDoubler und möchte die ID des Entwicklers nutzen. (In diesem Falle: <strong>Danke</strong>!)', 'wp-appbox'); ?></label></p>
					</div>
				</td>
			</tr>
			<tr>
