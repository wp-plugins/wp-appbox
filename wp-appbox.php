...
function get_app_author() {
	...
	foreach($ItemInfo['General'] as $appdata) {
		...
		if(trim($app_author) != '') {
			if((strlen($app_author) > 31) && !$options['wpAppbox_noqrcode']) $app_author = substr($app_title, 0, 29).'…';
			if((strlen($app_author) > 44) && $options['wpAppbox_noqrcode']) $app_author = substr($app_title, 0, 43).'…';
		}
		return($app_author);
	}
}
...
function wpAppbox_appbox($atts, $content) {
	...
	else return('<div class="appcontainer"><ul class="error"><li>'.__('Die App konnte <b>nicht</b> gefunden werden.', 'wp-appbox').' :-(</li></ul></div>');
}
