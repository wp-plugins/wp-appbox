...
Plugin URI: http://www.blogtogo.de/wp-appbox-app-badge-fuer-google-play-mac-app-store-windo/Users/marcel/Sites/blogtogo.de/wp-content/plugins/wp-appbox/__new_design
/Users/marcel/Sites/blogtogo.de/wp-content/plugins/wp-appbox/__old_designws-store-windows-phone-store-co/
...
function wpAppbox_urlmagic($content) {
	//$content = str_replace('TEXT_TO_REPLACE', 'TEXT_YOU_WANT_TO_REPLACE_WITH', $content);
	return $content;
}
add_filter('the_content', 'wpAppbox_urlmagic');
...
function wpAppbox_add_buttons($buttons) {
	...
	if(($option == '0') || ($option == '3')) {
		...
		if(($options['wpAppbox_button_alone_pebble']) || ($option == '0')) array_push($buttons, "separator", "wpAppbox_PebbleButton");
...
function wpAppbox_add_buttons_html() {
	global $store_names;
	$options = get_option('wpAppbox');
	$option = $options['wpAppbox_button_default'];
	if($option != '2') {
		if(wp_script_is('quicktags')) {
			...
			foreach($store_names as $store => $name) {
				if(($options['wpAppbox_button_html_'.$store]) || ($option == '0')) echo "QTags.addButton('htmlx_".$store."', 'Appbox: ".$store."', '[appbox ".$store." appid]', '', '', '".$name."');";
			}
			echo "</script>";
		}
	}
...
function wpAppbox_register($plugin_array) {
	...
	$plugin_path = strstr(plugin_dir_path(__FILE__), '/wp-content/');
	if(($option == '1') || ($option == '3')) {
		if(($iscombined) || ($option == '1')) {
			if(version_compare(get_bloginfo('version'), '3.9', '>=')) $plugin_array['wpAppbox_CombinedButton'] = get_home_url(null, $plugin_path . 'buttons/combined.btn');
			else $plugin_array['wpAppboxCombined'] = get_home_url(null, $plugin_path . 'buttons/combined.btn.old');
		}
	}
	if (($option == '0') || ($option == '3')) $plugin_array['wpAppboxSingle'] = get_home_url(null, $plugin_path . 'buttons/single.btn');
	return $plugin_array;
}
...
function wpAppbox_activate()
{
	if(version_compare(phpversion(), '5.0') == -1) {
		br_trigger_error(__('To use this plugin requires at least PHP version 5.0 is required.', 'wp-appbox')); 
	} 
	...
	if(!function_exists('curl_exec')){
		br_trigger_error(__('"curl_exec" is disabled on this server, but is required. Please enable this feature (or contact your hoster).', 'wp-appbox')); 
	}  
	//add_endpoint();
	//global $wp_rewrite;
...
function add_endpoint() {
	global $wp_rewrite;
	$plugin_path = strstr(plugin_dir_path(__FILE__), 'wp-content/');
	add_rewrite_rule($plugin_path.'buttons/combined\.btn$', 'index.php?wpappbox=combined', 'top');
	add_rewrite_rule($plugin_path.'buttons/combined\.btn\.old$', 'index.php?wpappbox=combined.old', 'top');
	add_rewrite_rule($plugin_path.'buttons/single\.btn$', 'index.php?wpappbox=single', 'top');
}
function handle_endpoint( $template ) {
	global $wp_query;
	if(!empty($wp_query->query_vars['wpappbox'])) {
		if('single' === $wp_query->query_vars['wpappbox']) return plugin_dir_path(__FILE__) . 'buttons/single.btn.js.php';
		else if('combined' === $wp_query->query_vars['wpappbox']) return plugin_dir_path(__FILE__) . 'buttons/combined.btn.js.php';
		else if('combined.old' === $wp_query->query_vars['wpappbox']) return plugin_dir_path(__FILE__) . 'buttons/combined.btn.old.js.php';
	}
	return $template;
}
function canonical($redirect) {
	$query_var = get_query_var('wpappbox');
	if(!empty($query_var)) return false;
	return $redirect;
}
add_filter('redirect_canonical', 'canonical');
add_filter('query_vars', 'add_query_var');
add_filter('template_include', 'handle_endpoint');
...
add_action('admin_print_footer_scripts', 'wpAppbox_add_buttons_html');
